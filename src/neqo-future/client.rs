...
impl Connection {
    fn configure_client(config: &ClientConfig, src_addr: SocketAddr, dst_addr: SocketAddr, sock_conn: Arc<UdpSocket>) -> Result<Connection, QuicError> {
...
pub async fn connect(conn_addr: SocketAddr, config: ClientConfig) -> Result<(JoinHandle<Option<u64>>, Connection), QuicError> {
